...
输出是一个固定格式的安全分析报告，clings 会捕获程序 stdout 与内置期望输出精确匹配（`clings tests 61` 查看）。
...
```
      ...
      +16           ├──────────────┤
                    │  保存的 rbp   │
      +0            ├──────────────┤
                    │  buf[0..15]   │ ← gets() 从这里开始写入 (16 bytes)
                    └──────────────┘
```
...
```
buf[0] 到返回地址的距离 = 16 (buf) + 8 (rbp) = 24 字节

x86-64: buf 位于 rbp-16，返回地址位于 rbp+8；buf[16] 已是 16 字节对齐，
无需额外填充。精确偏移随编译器版本/优化级别而变，生产中用 objdump/gdb 确认。
```
...
```
      ...
      +16           ├──────────────┤
                    │  保存的 rbp   │ ← fgets 无法到达！
      +0            ├──────────────┤
                    │  buf[0..15]   │ ← fgets 最多写 15+1 字节 (16 bytes)
                    └──────────────┘
```

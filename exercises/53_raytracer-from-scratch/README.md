...
用纯 C 语言从零实现一个基础光线追踪器（raytracer）：定义场景（2 个球体 + 1 个点光源 + 1 个相机），逐像素发射光线，计算光线 - 球体交点，应用 Phong 光照模型，输出 PPM P3 文本格式的 64×32 彩色图像。构建用 `make`；判分/自测用 `clings run`/`clings watch`（clings 会捕获程序 stdout 与内置测试用例逐行比对），期望输出可用 `clings tests 53` 查看。
...
64 32           ← 宽度 高度
255             ← 最大颜色值
0 0 0           ← 像素(0,0): 黑色背景 (顶部无物体, R=0 G=0 B=0)
0 0 0           ← 像素(1,0): 黑色背景
...             ← 顶部若干行都是背景; 只有光线击中球体处才有颜色
203 0 0         ← 例如红球顶端某像素 (R=203 G=0 B=0), 出现在中部行
...             ← 共 WIDTH×HEIGHT 行像素数据
```

> 注意：本图的第一个像素 (0,0) 是左上角，对应黑色背景 `0 0 0`；红/蓝球体位于图像中部，因此彩色像素出现在中间的若干行，而非文件开头。

PPM (Portable Pixmap) 是最简单的图像格式之一，纯文本，可以直接用 `cat` 查看，也便于逐行比对（这正是 clings 判分逐行匹配 stdout 的基础）。P3 表示 ASCII 格式（还有 P6 二进制格式）。
...
| 判分逐行比对失败      | 浮点精度导致微小差异           | 计算顺序与公式需与参考实现完全一致 |

> **浮点可移植性说明**：本题涉及 `sqrt`/`pow` 等浮点运算，不同编译器/CPU/优化等级下最低位可能有微小差异，导致 `to_byte` 取整后个别像素相差 1。本题判分以 **Ubuntu x86-64（gcc，默认优化）** 的输出为基准；只要严格按 README 的公式与计算顺序实现，在该环境下即可逐行匹配。若在其它平台自测出现极个别像素差异，通常是浮点舍入而非逻辑错误。
...
5. **为什么分辨率这么低？** 64×32 = 2048 条光线，在纯 CPU 上几乎瞬间完成。教学目的：小图像便于肉眼检查 PPM 文本输出，也方便逐行核对判分结果。提升分辨率只需改 WIDTH/HEIGHT。
